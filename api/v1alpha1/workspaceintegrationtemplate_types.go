...
type ResourceRef struct {
	// Name is the handle used to reference this resource in template expressions:
	// {{ resource "<name>" "<jsonpath>" }}. Must be unique within resourceRefs.
...
type IntegrationTemplateParameter struct {
	// Name of the parameter, referenced in template expressions as {{ .Parameters.<Name> }} and supplied
	// by the workspace as integrationTemplateRefs[].parameters[].name.
...
type WorkspaceIntegrationTemplateSpec struct {
	...
	// Parameters declares the parameters this template consumes via {{ .Parameters.<name> }}. It is the
	// single source of truth for the template's parameter contract: a template expression may only
	// reference a declared parameter (an undeclared {{ .Parameters.X }} is rejected at the template
	// author's write), and a referencing Workspace must supply every declared parameter. All declared
	// parameters are required -- there are no optional parameters or defaults, so an unsupplied one
	// would resolve to empty; the Workspace webhook rejects that at the user's write instead.
	// +listType=map
	// +listMapKey=name
	// +kubebuilder:validation:MaxItems=10
	// +optional
	Parameters []IntegrationTemplateParameter `json:"parameters,omitempty"`

	// ResourceRefs defines the resources to fetch at resolution time. Each entry is
	// addressed in template expressions by its handle name: {{ resource "<name>" "<jsonpath>" }}.
	// A template exists to resolve values from at least one referenced resource, so at
	// least one ref is required; capped at one for now (see integrationTemplateRefs).
	// +listType=map
	// +listMapKey=name
	...
	// StatusProbe is an optional probe the operator runs periodically
	...
	// single verdict surfaced in workspace.status.integrationStatuses[].
	//
