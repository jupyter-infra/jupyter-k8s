...
| `webSocketURLTemplate` _string_ | WebSocketURLTemplate constructs the WebSocket (ssh-over-websocket) connection URL; required to enable WebSocket connections. Template vars: .Workspace, .AccessStrategy. |  | Optional: \{\} <br /> |
