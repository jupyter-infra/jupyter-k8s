...
| `webSocketURLTemplate` _string_ | WebSocketURLTemplate constructs the WebSocket (ssh-over-websocket) connection URL; if unset, it falls back to BearerAuthURLTemplate with /bearer-auth stripped. Template vars: .Workspace, .AccessStrategy. |  | Optional: \{\} <br /> |
