...
// SafelyRemoveFinalizerFromAccessStrategy attempts to remove the named finalizer from an AccessStrategy,
// handling conflict errors by checking if the finalizer was removed in a concurrent operation.
//
// finalizerName selects which protection finalizer to manage (workspace vs template reference).
...
func SafelyRemoveFinalizerFromAccessStrategy(
	...
	if !controllerutil.ContainsFinalizer(accessStrategy, finalizerName) {
		logger.V(1).Info("Finalizer not present on AccessStrategy", "finalizer", finalizerName)
		return nil
	}

	// Remove the finalizer
	logger.Info("Removing finalizer from AccessStrategy", "finalizer", finalizerName)
	controllerutil.RemoveFinalizer(accessStrategy, finalizerName)

	// Try to update
	updateErr := k8sClient.Update(ctx, accessStrategy)
	if updateErr == nil {
		logger.Info("Successfully removed finalizer from AccessStrategy", "finalizer", finalizerName)
		return nil
	}
	...
	if !controllerutil.ContainsFinalizer(latestAccessStrategy, finalizerName) {
		logger.Info("Finalizer already removed in latest version of AccessStrategy", "finalizer", finalizerName)
		return nil
	}
