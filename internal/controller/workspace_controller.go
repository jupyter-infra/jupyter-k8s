...
func SetupWorkspaceController(mgr mngr.Manager, options WorkspaceControllerOptions) error {
	...
	// Integration status prober. The pod exec util needs the in-cluster rest config; if it can't
	// be built (e.g. outside a cluster), pass a nil prober -- the state machine's probe step
	// nil-guards and simply reports no integration status.
	...
	if execUtil, execErr := NewPodExecUtil(); execErr != nil {
		logf.Log.WithName("workspace-controller").Error(execErr,
			"failed to create pod exec util; integration status probing disabled")
	} else {
		integrationProber = NewIntegrationProber(k8sClient, execUtil)
	}

	stateMachine := NewStateMachine(resourceManager, statusManager, eventRecorder, idleChecker, accessStartupProber, integrationManager, integrationProber)
