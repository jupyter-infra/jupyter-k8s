...
var _ = Describe("EnvAdder", func() {
	...
	It("should do nothing when template has no AddEnv", func() {
	...
	It("should add all template env vars when workspace has none", func() {
		template.Spec.AddEnv = []corev1.EnvVar{
			{Name: "A", Value: "1"},
			{Name: "B", Value: "2"},
		}
	...
	It("should not override existing workspace env var by name", func() {
		...
		template.Spec.AddEnv = []corev1.EnvVar{{Name: "A", Value: "template-loses"}}
	...
	It("should deep copy template env vars to prevent mutation", func() {
		template.Spec.AddEnv = []corev1.EnvVar{{Name: "X", Value: "original"}}

		applyEnvDefaults(workspace, template)
		workspace.Spec.Env[0].Value = "mutated"

		Expect(template.Spec.AddEnv[0].Value).To(Equal("original"))
	})
})
