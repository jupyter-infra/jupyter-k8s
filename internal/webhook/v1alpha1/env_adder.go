...
// applyEnvDefaults merges template's AddEnv into workspace's Env.
// Workspace env vars take precedence by name (same pattern as addLabels).
func applyEnvDefaults(workspace *workspacev1alpha1.Workspace, template *workspacev1alpha1.WorkspaceTemplate) {
	if len(template.Spec.AddEnv) == 0 {
		return
	}
	...
	for _, e := range template.Spec.AddEnv {
