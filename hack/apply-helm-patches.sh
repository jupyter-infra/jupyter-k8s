...
# --- Create extension API auth binding ---
# Uses ClusterRole/ClusterRoleBinding instead of a RoleBinding in kube-system.
# The kube-system RoleBinding approach fails in environments where the installer
# lacks cross-namespace permissions (e.g., EKS addon framework).
